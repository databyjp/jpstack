...
The Product contract, task-boundary rule, and evidence mapping were implemented in commit `79b55c3`. Commit `07df5f1` added proportional verification selection and compressed this roadmap. The working tree adds the S09 case study, default execution packets, a checkpoint-review override, and nine execution-packet evaluation cases. The remaining work should proceed as separate executable checkpoints.

This handoff proposes independently testable improvements to `jpstack`. The main goal is to reduce low-value human supervision by moving decisions to the start of a task, grouping related decisions, and allowing in-envelope execution between human reviews. It does not propose a broad autonomous orchestrator or a sticky workflow router.
...
The S09 case study found that its 23-session lineage contained at least seven Product tasks. Of its 27 approval-only turns, 13 were conservative candidates for in-envelope continuation and 14 approved a new Product outcome or crossed an ask-first boundary. See `references/s09-bounded-autonomy-case-study.md`.

The evidence supports two conclusions:

1. Session lineage is not a task boundary. Do not use S09's 27 turns as a single-task autonomy baseline. Evaluate execution packets against the 13 reclassified continuation turns while preserving all 14 Product and ask-first gates.
...
Gated work uses one approved execution packet by default. The human may request checkpoint-by-checkpoint review without changing the packet's safety rules.

### 2. Design and evaluate execution packets

**Purpose:** Replace serial continuation approvals with one structured approval for eligible multi-checkpoint work.
...
4. **Completed:** Make one execution packet the default for Gated work. The packet contains the Product contract, allowed scope, permitted actions, validation plan, ask-first conditions, external-effect permissions, and retry and no-progress budgets. A human can request checkpoint-by-checkpoint review as a cadence override.
...
An execution packet applies only while the current Pi run remains active. Execution must interrupt for user-visible behavior changes, public interfaces, persisted schemas, security or privacy decisions, credentials, meaningful spend, irreversible or externally visible effects, material scope expansion, or choices that make retained work expensive to reject.

**Primary assertion:** An eligible task completes several in-envelope checkpoints after one packet approval, while every seeded ask-first decision interrupts execution and a checkpoint-review request stops after each checkpoint.

**Synthetic result:** The prior named-mode policy passed 7 of 9 S09-derived execution-packet cases; the unified policy passed 9 of 9. All 12 existing controls passed. The unified policy treated checkpoint-by-checkpoint review as a cadence override and requested a new packet, not another mode choice, at a new Product outcome. No seeded Product, schema, security, external-effect, or no-progress stop was suppressed. This supports a real-work trial of the default packet policy.

### 3. Trial execution packets on real multi-checkpoint Product tasks

Use the default execution packet on 5 to 10 Product tasks expected to require several checkpoints. Record any task where the human requests checkpoint-by-checkpoint review.

Measure in-envelope continuation turns, preserved Product and ask-first gates, substantive decisions, scope corrections, false completion, reviewer-requested rework, validation coverage, and incomplete tasks. Keep the default only if removable continuation turns fall by at least 50% without suppressing a required gate or increasing scope corrections, false completion, or rework.
...
Begin this phase only if the execution-packet trial succeeds. Implement its parts as separate checkpoints:
...
`references/s09-bounded-autonomy-case-study.md` records the result. The 23 linked sessions contained at least seven Product tasks, so the original grouping overstated one task's continuation burden. The study reclassifies 13 of 27 approval-only turns as in-envelope continuations, preserves 14 Product or ask-first gates, and defines sanitized fixtures for task boundaries, security stops, persisted outcome semantics, stale evidence, grouped external-run approval, and no-progress stops.
...
1. Trial default execution packets on real Product tasks that need several checkpoints and record checkpoint-review overrides.
...
| In-envelope continuation turns | S09 had 13 after reclassification | Reduce by at least 50% |
